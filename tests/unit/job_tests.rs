...
fn test_job_with_config() {
    let job = Job::with_config(
        "process_payment",
        json!({ "order_id": "order_123", "amount": 100.50 }),
        "payments",
        10,
        5,
    );
    ...
    assert_eq!(job.payload["order_id"], "order_123");
...
fn test_job_new_typed() {
    ...
    assert_eq!(job.method, "greet");
    assert_eq!(job.payload["name"], "alice");
    assert_eq!(job.payload["count"], 3);
}

#[test]
fn test_job_serialization() {
    let job = Job::new("test_method", json!({ "a": "arg1", "b": "arg2" }));
...
fn test_job_serialization_roundtrip() {
    let mut job = Job::new("complex_job", json!({ "arg": "arg1" }));
...
fn test_job_state_transitions() {
    let mut job = Job::new("test", Value::Null);
...
#[test]
fn test_job_age() {
    let job = Job::new("test", Value::Null);
...
fn test_clone_with_new_id() {
    let mut original = Job::new("test_method", json!({ "arg": "arg1" }));
...
fn test_job_equality() {
    let job1 = Job::new("test", json!({ "arg": "arg" }));
    let job2 = Job::new("test", json!({ "arg": "arg" }));
...
#[test]
fn test_job_with_null_payload() {
    let job = Job::new("no_args_method", Value::Null);
